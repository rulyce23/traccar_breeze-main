...
<?php $component->withAttributes([]); ?>
    <div class="flex ml-[25px]">
        <div class="min-h-[88vh] w-full max-w-[1200px] border-r border-gray-300 bg-white p-6">
            <!-- Search Form -->
            <form method="GET" action="<?php echo e(route('device.index')); ?>" class="flex flex-wrap gap-4 items-center">
                    ...
                    <tbody>
                        <?php $__empty_1 = true; $__currentLoopData = $devices; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $device): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            <tr class="border-b hover:bg-gray-50">
                                ...
                                <td class="px-4 py-2"><?php echo e($device->user->name ?? ''); ?></td> <!-- Display user name -->
                                <td class="px-4 py-2"><?php echo e($device->name); ?></td>
                                <td class="px-4 py-2"><?php echo e(isset($device->attributes) ? $device->attributes->vehicle_name ?? '' : ''); ?></td>
                                <td class="px-4 py-2"><?php echo e($device->uniqueId); ?></td>
                                <td class="px-4 py-2"><?php echo e($device->model); ?></td>
                                <td class="px-4 py-2"><?php echo e($device->status); ?></td>
                                <td class="px-4 py-2"><?php echo e(isset($device->lastUpdate) ? \Carbon\Carbon::parse($device->lastUpdate)->toDateTimeString() : 'N/A'); ?></td>
                                <td class="px-4 py-2 flex gap-2">
                                <a href="#" onclick="openModals()"><i class="fa fa-edit text-blue-500 hover:text-blue-600"></i></a>
                                <a href="#"  onclick="openModal(<?php echo e($device->id); ?>)"><i class="fa fa-eye text-green-500 hover:text-green-600"></i></a>
                                <form action="<?php echo e(route('device.destroy', $device->id)); ?>" method="POST" onsubmit="return confirm('Are you sure you want to delete this device?')" style="display: inline;">
                                    <?php echo csrf_field(); ?>
     ...
     <div id="deviceModals" class="fixed inset-0 bg-gray-500 bg-opacity-75 flex items-center justify-center z-50 hidden">
        <div class="bg-white p-6 rounded-md shadow-lg w-11/12 sm:w-1/2">
            ...
            <form id="deviceForm" action="<?php echo e(route('device.store')); ?>" method="POST">
                <?php echo csrf_field(); ?>
    ...
    <div class="bg-white p-6 rounded-md shadow-lg w-11/12 sm:w-1/2">
        <h2 class="text-lg font-semibold mb-4" id="modalTitle">Device Details</h2>
        <form id="deviceForm" action="<?php echo e(route('device.update', 'id')); ?>" method="POST">
         <?php echo csrf_field(); ?>
...
<?php /**PATH C:\xampp\htdocs\traccar_breeze-main\resources\views/master/device/index.blade.php ENDPATH**/ ?>